...
def update(world: World, desired_velocities: np.ndarray, dt: float) -> None:
    ...
    steering = _clamp(steering, max_forces[mask])
    steering /= masses[mask, np.newaxis]

    world.velocities[mask] += steering * dt
    world.velocities[mask]  = _clamp(world.velocities[mask], max_speeds[mask])
